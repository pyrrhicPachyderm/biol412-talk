...
<<include=FALSE>>=
	...
	block_matrix <- function(row_groups, col_groups, lines=FALSE, label_lines=lines) {
		...
		get_species_column_label <- function(col_species_num) {
			alignment <- ifelse(col_species_num > 1 & is_begin_block(col_species_num, col_groups) & label_lines,
				"|c",
				"c"
			)
		...
		get_row_line <- function(row_species_num) {
			...
			index_cline <- ifelse(row_species_num > 1 & is_begin_block(row_species_num, row_groups) & label_lines,
				paste0("\\cline{1-1}"),
				""
			)
			row_line <- paste(c(
