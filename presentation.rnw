...
<<include=FALSE>>=
	...
	block_matrix <- function(row_groups, col_groups, full_interior=FALSE, inclusion_matrix=NULL, lines=FALSE, label_lines=lines) {
				...
				unlist(sapply(1:length(col_groups),
					function(col_group_index) {
						alpha_value <- ifelse(inclusion_matrix[row_group_index,col_group_index],
							paste0("$\\alpha_{",row_group_index,col_group_index,"}$"),
							""
						)
...
\end{frame}
\note[itemize]{
	\item In community ecology, we look at species interactions to understand community dynamics.
...
\begin{frame}{Incomplete Data}
	...
	\only<+>{
		\begin{align*}
			\Sexpr{block_matrix(row_groups, col_groups, inclusion_matrix=inclusion_matrix, lines=T)}
		\end{align*}
	}
	
	\only<+>{
		\begin{align*}
			\Sexpr{block_matrix(row_groups, col_groups, lines=T)}
		\end{align*}
	}
\end{frame}
\note[itemize]{
	\item The ultimate goal, all going well, is to write an R package.
