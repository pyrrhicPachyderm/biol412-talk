...
<<include=FALSE>>=
	...
	block_matrix <- function(row_groups, col_groups, full_interior=FALSE, lines=FALSE, label_lines=lines) {
		...
		begin <- paste(c("\\begin{array}{cc", rep(alignment, num_species_cols), "c}"), collapse="")
		end <- "\\end{array}"
		...
		get_species_column_label <- function(col_species_num) {
			...
			return(paste0("\\multicolumn{1}{",divider,alignment,"}{",col_species_num,"}"))
		}
			...
			row_line <- paste(c(
				...
				unlist(sapply(1:length(col_groups),
					function(col_group_index) {
						...
						paste0("\\multicolumn{",col_groups[col_group_index],"}{",divider,alignment,"}{",multirow,"}")
					}
				)),
				rdelim
			), collapse="&")
