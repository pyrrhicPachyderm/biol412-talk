...
<<include=FALSE>>=
	...
	block_matrix <- function(row_groups, col_groups, full_interior=FALSE, lines=FALSE, label_lines=lines) {
		...
		alignment <- function(num_cols) {
			paste0(">{\\centering\\arraybackslash}p{\\dimexpr",num_cols,"\\colwidth+",2*(num_cols-1),"\\arraycolsep\\relax}")
		}
		
		begin <- paste(c("\\begin{array}{cc", rep(alignment(1), num_species_cols), "c}"), collapse="")
		end <- "\\end{array}"
		
		get_species_column_label <- function(col_species_num) {
			divider <- ifelse(col_species_num > 1 & is_begin_block(col_species_num, label_col_groups) & label_lines,
				"|",
				""
			)
			return(paste0("\\multicolumn{1}{",divider,alignment(1),"}{",col_species_num,"}"))
		}
			...
			row_line <- paste(c(
				...
				unlist(sapply(1:length(col_groups),
					function(col_group_index) {
						...
						divider <- ifelse(col_group_index != 1 & lines,
							"|",
							""
						)
						paste0("\\multicolumn{",col_groups[col_group_index],"}{",divider,alignment(col_groups[col_group_index]),"}{",multirow,"}")
					}
				)),
				rdelim
			), collapse="&")
